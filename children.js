var DB_CHILDREN = [
    ...
    {
        "id": "drug_provoc",
        "title": "药物激发试验 (Drug Provocation Test) 临床实施极高危操作规范",
        "type": "reference_table",
        "输出": "确诊变态反应的风险流程约束",
        "输入": [],
        "计算结果": "function(v){ return '本量表为静态参考表，请查看下方标准数据'; }",
        "参考表": [
            { "临床操作环节": "1. 核心适应证 (何时可做)", "规范要求与说明": "1. 病史高度怀疑过敏，但皮肤针刺/皮内试验及血清特异性IgE均阴性，为彻底排除过敏标签；2. 为找到一种绝对安全的替代类药物；3. 确诊必须用该药，为脱敏治疗(Desensitization)确立基线。" },
            { "临床操作环节": "2. 【绝对禁忌证】 (严禁进行)", "规范要求与说明": "患者曾因任何药物发生过危及生命的重症过敏，如：过敏性休克、Stevens-Johnson综合征(SJS)、中毒性表皮坏死松解症(TEN)、DRESS综合征、严重药物引发的细胞减少症及脏器衰竭。违规操作有致死风险！" },
            { "临床操作环节": "3. 患者受试前准备", "规范要求与说明": "1. 必须在基础疾病(如哮喘、心脏病)处于完全稳定期进行；2. 提前停用干扰救援的药物(如β受体阻滞剂、ACEI)；3. 停用掩盖症状的药物：抗组胺药停3-7天，系统性糖皮质激素视半衰期停用。" },
            { "临床操作环节": "4. 场地与抢救设施要求", "规范要求与说明": "【必须】在具有高级生命支持(ALS)能力的重症监护室或专科门诊开展。床旁触手可及：肾上腺素(1:1000)、气管插管设备、除颤仪、吸氧装置，且必须预先建立静脉通道。" },
            { "临床操作环节": "5. 剂量递增滴定策略 (单盲口服法为例)", "规范要求与说明": "从极微量起始(通常是常规单次治疗量的 1:1000 或 1:100)。如果无反应，每隔 30 ~ 60 分钟剂量加倍或成对数级递增(例如 1% -> 10% -> 20% -> 剩余全量)。总累积量达到该药物单次常规治疗极量。" },
            { "临床操作环节": "6. 观察及终止指标", "规范要求与说明": "一旦出现任何客观过敏体征(如荨麻疹爆发、喘鸣音、血压骤降、喉头水肿感)，【立即终止试验】并就地按过敏性休克抢救。试验结束后至少留观2小时；部分易致迟发反应的药物需留观24小时。" }
        ],
        "公式解读": "药物激发试验被誉为确诊药物过敏的“金标准”，但因其本质是蓄意诱发过敏反应，伴有引发致死性过敏性休克的巨大风险。必须由经验丰富的变态反应专科医师操作，并签署极其详尽的医患知情同意书。",
        "category": "变态反应科/儿科",
        "适用人群": "疑难复杂药物过敏需排查者",
        "临床场景": "变态反应科(过敏科)病房/ICU"
    },
    {
        "id": "infant_food",
        "title": "婴儿添加辅食的步骤与方法",
